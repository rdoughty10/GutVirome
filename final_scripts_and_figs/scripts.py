...
def calculate_viral_families_comp(df, threshold):
    viral_families = df.columns.difference(['filename', 'Group', 'Individual'])
    comp = df[viral_families].div(df[viral_families].sum(axis=1), axis=0)
    return (comp > threshold).sum(axis=1)
...
def calculate_viral_richness_comp(df, threshold):
    viral_families = df.columns.difference(['filename', 'Group', 'Individual'])

    # 1. Convert to compositional (row-normalized)
    comp = df[viral_families].div(df[viral_families].sum(axis=1), axis=0).fillna(0)

    def shannon_index_comp(proportions):
        # 2. Apply threshold on proportions
        p = proportions[proportions > threshold]
        if p.empty:
            return 0.0
    ...
    return comp.apply(shannon_index_comp, axis=1)

def calculate_viral_richness_simpson_comp(df, threshold):
    viral_families = df.columns.difference(['filename', 'Group', 'Individual'])

    # 1. Convert to compositional (row-normalized)
    comp = df[viral_families].div(df[viral_families].sum(axis=1), axis=0).fillna(0)

    def simpson_index_comp(proportions):
        # 2. Apply threshold on proportions
        p = proportions[proportions > threshold]
        if p.empty:
            return 0.0

        # 3. Simpson's Index: 1 - sum(p_i^2)
        return 1.0 - np.sum(p ** 2)

    return comp.apply(simpson_index_comp, axis=1)
